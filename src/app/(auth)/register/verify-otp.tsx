...
export default function ConfirmPhoneScreen() {
  ...
  // Timer starts at 0 — "Resend now" is shown immediately on first arrival.
  // The timer only starts (resets to 120) when the user taps "Resend now".
  ...
  // Countdown timer — only runs when timer > 0
  useEffect(() => {
    if (timer <= 0) return;
  ...
  const handleVerify = () => {
    // TODO: navigate to next registration step once subsequent screens are built
